...
> `worklog query --issue-id HIVEDEV-3234` → `"started": "2026-06-01T12:52:00.000+0300"`
...
> `worklog query --issue-id HIVEDEV-3234 --started-after 1780261199999 --started-before 1780347600000`
...
> `worklog query --issue-id K2227759-141` → 5 rows, all authored by a colleague, none by the caller.
...
first — on `EAK21GEOO-281` that is a colleague, not the caller. A wrong accountId poisons the day
...
Never use `workitem get`.worklog — it is hard-capped at `maxResults 20` (`CXOPS-1` has 39). Never
...
> `worklog query --issue-id CXOPS-17` → `comment: {type:"doc", version:1, content:[{type:"paragraph",
> content:[{type:"text", text:"fix the ivr with bulk"}]}]}`
> All 12 rows on `HIVEDEV-3234` have no `comment` key at all.
...
`--override-editable` (18/18 worklogs on `U2633237-39` postdate its resolution date).
